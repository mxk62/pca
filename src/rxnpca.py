...
for chem_rec in sample.get():
    ...
    # Check if the chemical has a disconnected structure, e.g. c1cc([O-].[Na+])ccc1
    # Ignore chemicals with disconnected structures now as we use split('.') to 
    # identify multiple products/reactants in the reaction smiles.
    smiles = [s for s in smi.split('.')]
    if len(smiles) > 1:
        dc_chem += 1
        continue
    try:
...
print 'disconnected chemicals'
print dc_chem
print "Reaction stats:"
print "published: total, valid, accepted"
print old_stats['total'], old_stats['valid'], old_stats['accepted']
print "unpublished: total, valid, accepted"
print new_stats['total'], new_stats['valid'], new_stats['accepted']

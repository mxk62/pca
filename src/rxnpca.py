...
keys = ['accepted', 'valid', 'total', 'multiple', 'matched']
...
for chem_rec in sample.get():
    ...
    for t in transforms:
        ...
        for smi in set(possible).difference(reactions):
            try:
                ...
                continue
            new_stats['valid'] += 1
            # Check unpublished reactions with multiple products. 
            if len(rxn.prod_smis) == 1:
                new_rxns[smi] = rxn
                new_stats['accepted'] += 1
            else:
                new_stats['multiple'] += 1
    ...
    for uid in chem_rec['reactions']['produced']:
        ...
        if old_smi is not None:
            ...
            # Commenting out the for loop above and writing the loop below to 
            # go back to the previous approach of adding all published reactions
            # leading to the products.
            for smi in new_rxns.keys():
                if smi == old_rxn.smiles:
                    old_stats['matched'] += 1 # matched counts the published reactions
                    # that have exactly same smiles as generated by retrostep.
                    del reactions[smi] # if we already have an exact smiles we 
                    # delete it and add all published reactions next.
            if len(old_rxn.prod_smis) == 1:
                reactions[old_rxn.smiles] = old_rxn
                old_stats['accepted'] += 1
            else:
                old_stats['multiple'] += 1


print "Reaction stats:"
print "published: total, valid, accepted, matched, multiple"
print old_stats['total'], old_stats['valid'], old_stats['accepted'], old_stats['matched'], old_stats['multiple']
print "unpublished: total, valid, accepted, multiple"
print new_stats['total'], new_stats['valid'], new_stats['accepted'], new_stats['multiple']

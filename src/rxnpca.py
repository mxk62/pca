...
# Get the statistics regarding how many reactions were generated by a given
# transform in total, and how many of them are published ones, total 
# in degree and out degree of reactants and products.
#
# We are storing those data in a dictionary where keys are transform ids and
# values are pairs of possible reactions and published reactions numbers
# respectively.
#
# in degree and out degree data are stored in rstat(reactants statistics) and 
# pstat(products statistics) dictionaries. keys are reaction smiles(available 
# for both published and unpublished reactions) and values are
# pairs of in_degree and out_degree of reactants and products.  
tstat = {}
rstat = {}
pstat = {}
for rxn in reactions.values():
    tstat.setdefault(rxn.tid, [0, 0])
    tstat[rxn.tid][0] += 1
    if rxn.rxnid is not None:
        tstat[rxn.tid][1] += 1
        
    rstat.setdefault(rxn.smiles, [0, 0])
    pstat.setdefault(rxn.smiles, [0, 0])
    reactants = chemicals.find({'smiles': {'$in': rxn.react_smis}})
    products = chemicals.find({'smiles': {'$in': rxn.prod_smis}})
    for r in reactants:
        rstat[rxn.smiles][0] += r['in_degree']
        rstat[rxn.smiles][1] += r['out_degree']
    for p in products:
        pstat[rxn.smiles][0] += p['in_degree']
        pstat[rxn.smiles][1] += p['out_degree']
...
for rxn in reactions.values():
    ...
    r_in_degree = rstat[rxn.smiles][0]
    p_in_degree = pstat[rxn.smiles][0]
    r_out_degree = rstat[rxn.smiles][1]
    p_out_degree = pstat[rxn.smiles][1]
    auxiliaries.append([rxn.rxnid, rxn.tid, rxn.year, rxn.popularity, 
                        possible, published, r_in_degree, p_in_degree, 
                        r_out_degree, p_out_degree, status])
